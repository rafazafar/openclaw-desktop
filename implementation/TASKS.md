...
- [x] **T3.1 Generated OpenClaw config (MVP)** ✅ 2026-02-04 — Manager now writes `openclaw.generated.json` to app data on Telegram connect/disconnect; includes `meta.generatedBy` and no secret token.
  - Manager generates a minimal OpenClaw config file/artifact.
  - Include header: generated by openclaw-desktop.

- [x] **T3.2 Safe writes + rollback** ✅ 2026-02-04 — Implemented atomic write (tmp→fsync→rename) with single `.bak` rollback for `state.json` and `openclaw.generated.json`; added a vitest.
...
- [x] T6.2 OAuth flow + token storage ✅ 2026-02-04 — Added Gmail OAuth start/callback endpoints with local public callback + token exchange/store (dev/plaintext); desktop UI can launch auth + clear tokens.
- [x] T6.3 Gmail read-only integration wiring ✅ 2026-02-04 — Added Gmail connection summary (account email) + `/status` wiring; generated config now includes `integrations.gmail` with tokenRef + allowRead/allowSend gates; added tests.
